...
def test_list_library_with_metadata(tmp_path, capsys):
    ...
    metadata1 = {"title": "Test Song 1", "id": "clip123", "tags": ["pop", "electronic"]}
    metadata2 = {"title": "Test Song 2", "clip_id": "clip456"}
...
def test_export_text_with_files(tmp_path, capsys):
    ...
    metadata = {
        ...
        "handle": "testuser",
        "is_liked": True,
        "metadata": {"prompt": "Test lyrics content", "tags": "pop, electronic"},
    }
    (date_dir / "test123_metadata.json").write_text(json.dumps(metadata))

    # Create non-liked track (should be ignored)
    metadata2 = {"title": "Unliked Song", "id": "test456", "is_liked": False}
