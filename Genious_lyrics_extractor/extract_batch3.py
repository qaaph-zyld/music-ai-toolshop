"""Batch 3 lyrics extraction from Genius for Devito, TNG, Voyage, Rasta, Maya Berovic, Ana Nikolic, Breskvica, Henny.
...
ARTISTS = [
    # ── Drill/Trap cohort ──────────────────────────────────────────────
    {
        "name": "Devito",
        "folder": "devito",
        "variants": {"devito", "david ljubenović", "david ljubenovic"},
    },
    {
        "name": "TNG",
        "folder": "tng",
        "variants": {"tng", "tajnekigas", "aleksandar gavrić", "aleksandar gavric"},
    },
    {
        "name": "Voyage",
        "folder": "voyage",
        "variants": {"voyage", "mihajlo veruović", "mihajlo veruovic"},
    },
    {
        "name": "Rasta",
        "folder": "rasta",
...
def rebuild_index(outdir: Path) -> dict[str, int]:
    """Rebuild _index_batch3.json and _summary_batch3.md from existing files on disk."""
    ...
    index_path = outdir / "_index_batch3.json"
    with index_path.open("w", encoding="utf-8") as f:
        json.dump(index, f, indent=2, ensure_ascii=False)
    print(f"Index saved: {index_path} ({len(index)} unique entries)")

    dedup_path = outdir / "_dedup_log_batch3.json"
    ...
    summary_path = outdir / "_summary_batch3.md"
    with summary_path.open("w", encoding="utf-8") as f:
        f.write("# Batch 3 Lyrics Extraction Summary (rebuilt from disk)\n\n")
...
def main():
    ...
    parser.add_argument(
        "--rebuild",
        action="store_true",
        help="Rebuild _index_batch3.json and _summary_batch3.md from existing files on disk (no API calls)",
    )
    ...
    # Print summary
    print(f"\n{'='*60}")
    print("BATCH 3 EXTRACTION SUMMARY")
    print(f"{'='*60}")
    ...
    summary_path = outdir / "_summary_batch3.md"
    with summary_path.open("w", encoding="utf-8") as f:
        f.write("# Batch 3 Lyrics Extraction Summary\n\n")
        f.write(f"**Date:** {time.strftime('%Y-%m-%d %H:%M')}\n")
        f.write(f"**Artists:** {', '.join(a['name'] for a in ARTISTS)}\n\n")
        f.write("## Statistics\n\n")
        f.write("| Category | Count |\n|----------|-------|\n")
