...
class SunoLibrary:
    ...
    def __init__(self, library_path: Optional[Path] = None):
        ...
        self.db_path = self.library_path / "suno_library.db"
        self._cache: List[SunoTrack] = []
        self._load_from_db()
    
    def _load_from_db(self):
        """Load track metadata from SQLite database"""
        if not self.db_path.exists():
            # Fallback to stub if no database
            self._cache = [
                SunoTrack(
                    id="demo_001",
                    title="Demo Track 1",
        ...
        try:
            ...
            # Query songs with their tags
            cursor.execute("""
                SELECT s.id, s.title, s.artist, s.bpm, s.musical_key, 
                       s.local_audio_path, s.url, s.lyrics
                FROM songs s
            """)
            ...
            for row in rows:
                song_id, title, artist, bpm, musical_key, audio_path, url, lyrics = row
                
                # Get tags for this song
                cursor.execute("""
                    SELECT tag FROM tags WHERE song_id = ?
                """, (song_id,))
                tags = [t[0] for t in cursor.fetchall()]
                
                # Use bpm for tempo, default to 120 if null
                tempo = int(bpm) if bpm else 120
                key = musical_key if musical_key else "C"
                genre = tags[0] if tags else "unknown"
                
                # Use local path if available, otherwise use url as identifier
                path = Path(audio_path) if audio_path else (Path(url) if url else self.library_path / f"{song_id}.mp3")
                
                self._cache.append(SunoTrack(
                    id=song_id,
