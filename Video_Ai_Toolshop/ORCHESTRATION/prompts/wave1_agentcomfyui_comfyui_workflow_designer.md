...
3. WAIT FOR MY TASK.
4. Call `start_session` MCP tool or run: python scripts/session.py brief "Design ComfyUI workflow JSON for Wan 2.2 I2V with camera control (TTM or Fun Camera Control), ControlNet stacking, and post-processing nodes. Pin ComfyUI version. Validate against /object_info endpoint." --files "d:/Project/Video_Ai_Toolshop/workflows/".
...
MY TASK: Design ComfyUI workflow JSON for Wan 2.2 I2V with camera control (TTM or Fun Camera Control), ControlNet stacking, and post-processing nodes. Pin ComfyUI version. Validate against /object_info endpoint.
