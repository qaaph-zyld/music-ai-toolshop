...
3. WAIT FOR MY TASK.
4. Call `start_session` MCP tool or run: python scripts/session.py brief "Generate 3-5 cinematic reference images per target scene (10 scenes = 30-50 images) using Flux + LoRA + Five Pillars prompt framework. Output at 1024x576 (16:9) to match Wan 2.2 input. Use lora_scale 0.8-1.2 at inference (NOT fuse_lora). Save all images with metadata." --files "d:/Project/Video_Ai_Toolshop/output/reference_images/".
...
MY TASK: Generate 3-5 cinematic reference images per target scene (10 scenes = 30-50 images) using Flux + LoRA + Five Pillars prompt framework. Output at 1024x576 (16:9) to match Wan 2.2 input. Use lora_scale 0.8-1.2 at inference (NOT fuse_lora). Save all images with metadata.
...
CONSTRAINTS:
- Depends on Wave 2 flux_lora. Generate at Wan 2.2 aspect ratio.
