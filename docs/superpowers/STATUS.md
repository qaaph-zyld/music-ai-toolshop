...
> | **M5 MEASURED, not closed** | large-v3 installed and measured: **min/track ~3.6–3.8**, RTF 1.09–1.17× (beats realtime), **7.0% repeat drift — the first valid timing** after two void attempts at 27–32%. `temperature=0` was the fix: faster-whisper's fallback ladder made the module nondeterministic (154 vs 194 words on identical input); disabling it gave **byte-identical output**, ~35% more speed, coverage 62%→69% and longest span 39.0→22.3 s. Corpus estimate: ~~**~25 h CPU for the 444 dossiers**~~ **CORRECTED 2026-09-01 - the corpus is **222** dossiers, not 444 (`*_analysis.json` also matched the `_voice_analysis.json` sidecar, double-counting every track). Real audio ~13.6 h over 221 tracks, so transcription is **~13 h - an overnight**. See `JOURNAL.md` J-020/J-021. **Still open on fitness:** 69% coverage / 45 words-per-min vs rap's 100–200 — a third of the vocal yields no timings. |
> | **Two measurement traps caught here** | *Confidence is not correctness* — 0.836 mean word probability while dropping 43% of the track. *A clip screen overstated the fix* — a 90 s A/B promised +23 pts coverage and a 22.3→5.2 s span collapse; the full track gave **+5 pts and a worse span**. Same shape as the earlier "1.40×". |
