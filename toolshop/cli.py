...
def build_parser() -> argparse.ArgumentParser:
    ...
    lyrics_stats_parser.add_argument(
        ...
        help="Filter to a genre cohort (drill_trap or pop), excluding featured songs",
    )
    lyrics_stats_parser.add_argument(
    ...
    lyrics_transform_parser.add_argument(
        ...
        choices=["vocabulary", "slang", "structure", "flow", "rhyme", "all"],
        help="Transformation direction: vocabulary, slang, structure, flow, rhyme, or all (default: all)",
    )
    lyrics_transform_parser.add_argument(
        "--mode", type=str, default="report",
        choices=["report", "auto-fix", "interactive"],
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    # =========================================================================
        ...
        if args.lyrics_command == "fetch":
            ...
            if args.cohort:
                artists = get_cohort_stats(conn, args.cohort)
                if args.json:
                    print(json_mod.dumps(artists, indent=2, ensure_ascii=False))
                else:
                    if not artists:
                        print(f"No data found for cohort: {args.cohort}")
                        conn.close()
                        return
                    print(f"\nCohort: {args.cohort} (featured songs excluded)")
            ...
            directions = ["vocabulary", "slang", "structure", "flow", "rhyme"] if args.direction == "all" else [args.direction]
